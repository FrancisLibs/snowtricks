...
class TricksFixtures extends Fixture
{
    ...
    public function __construct(UserPasswordEncoderInterface $passwordEncoder, CategoryRepository $repository)
    {
    ...
    public function load(ObjectManager $manager)
    {
        ...
        $tricksNames = array(
            'FrontFlip', 'BackFlip', '360', 'Reverso', 'Regular', 'Goofy', 'graber', 'Backside air',
        ...
        foreach ($users as $username) {
            ...
            for($cptTricks =1; $cptTricks<=mt_rand(1, 2); $cptTricks++)
            {
                // Creation d'une catégorie
                $categories = array('flips', 'grabs', 'spins', 'slides', 'rotations', 'oneFoot');
                $category = new Category();
                $category->setName($categories[array_rand($categories, 1)]);
                $manager->persist($category);

                $descript = "<p>" . join("</p><p>", $faker->paragraphs(3)) . "</p>";
    
                $trick = new Trick();
                $indexLecture = mt_rand(1, 6);
                $category = $this->repository->findONeBy([], [], 1, $indexLecture);
                $index = rand(1, 10);
                ...
                // ajout de 2 à 4 photos
                for ($cptPictures = 1; $cptPictures <= mt_rand(2, 5); $cptPictures++)
                {
                    $picture = new Picture();
                    $index = rand(1, 5);
                    $picture->setFilename($category->getName().$index . '.jpg')
                            ->setTrick($trick);                        
